...
`crew_id`를 기본키로 가지고 `nickname`을 컬럼으로 가지는 테이블을 만든다.
...
WHERE crew_id = (SELECT crew_id FROM crew WHERE nickname = '주니') AND attendance_date = '2025-03-12';
```
...
WHERE crew_id = (SELECT crew_id FROM crew WHERE nickname = '아론') AND attendance_date = '2025-03-12';
```
...
#### 검프는 SQL이 익숙지 않아 `crew` 테이블에서 먼저 닉네임을 검색하고 해당 아이디 값을 찾아 직접 WHERE문에서 crew_id 항목의 값을 수동으로 입력했다. 그런데 `nickname`을 입력하면 이를 기준으로 쿼리문을 처리할 수도 있지 않을까?
